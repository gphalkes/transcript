...
static lt_dlhandle do_dlopen(const char *file) { return lt_dlopen(file); }
...
static transcript_t *open_converter(const char *normalized_name, transcript_utf_t utf_type,
  ...
  if ((handle = _transcript_db_open(normalized_name, "ltc", (open_func_t)do_dlopen, error)) ==
      NULL) {
    FILE *test_handle;
...
*/
transcript_t *transcript_open_converter_nolock(const char *name, transcript_utf_t utf_type,
                                               int flags, transcript_error_t *error) {
  transcript_name_desc_t *converter;
  char normalized_name[NORMALIZE_NAME_MAX];
...
*/
static FILE *db_open(const char *name, const char *ext, const char *dir, open_func_t open_func,
...
/** @internal
    ...
    directory.
*/
void *_transcript_db_open(const char *name, const char *ext, open_func_t open_func,
                          transcript_error_t *error) {
  const char **next_dir;
  FILE *result;
