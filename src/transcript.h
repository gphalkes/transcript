...
enum transcript_flags_t {
  ...
  TRANSCRIPT_SUBST_UNASSIGNED =
      (1 << 1), /**< Automatically replace unmappable characters by substitute characters. */
  TRANSCRIPT_SUBST_ILLEGAL =
      (1 << 2), /**< Automatically insert a substitution character on illegal input. */
  TRANSCRIPT_ALLOW_PRIVATE_USE =
      (1 << 3), /**< Allow private-use mappings. If not allowed, they are handled like unassigned
                   sequences, with the exception that they return a different error.. */

  /* These are only valid as argument to transcript_from_unicode and transcript_to_unicode. */
  TRANSCRIPT_FILE_START = (1 << 8), /**< The begining of the input buffer is the begining of a file
                                       and a BOM should be expected/generated. */
  ...
  */
  ...
  /** Only convert the next character, then return (useful for handling fallback/unassigned
     characters etc).
  ...
  */
  ...
  /* NOTE: internal flags are defined in transcript_internal.h and moduledefs.h. Make sure these
     don't overlap! */
};
...
typedef enum {
  ...
  TRANSCRIPT_INCOMPLETE, /**< The buffer ended with an incomplete sequence, or more data was needed
                            to verify a M:N conversion. */
  ...
  TRANSCRIPT_PACKAGE_FILE,       /**< The converter name references a converter package file, not an
                                    actual converter. */
  ...
  TRANSCRIPT_PART_SUCCESS_MAX =
      TRANSCRIPT_INCOMPLETE /**< Highest error code which indicates success or end-of-buffer. */
...
typedef struct {
  ...
  /** A boolean indicating whether the converter is available.
      ...
      @note If availability is indicated, a load failure may still occur if the conversion table
      is corrupt. */
  int available;
} transcript_name_t;
...
TRANSCRIPT_API transcript_t *transcript_open_converter(const char *name, transcript_utf_t utf_type,
...
TRANSCRIPT_API transcript_error_t transcript_from_unicode(transcript_t *handle, const char **inbuf,
                                                          const char *inbuflimit, char **outbuf,
                                                          const char *outbuflimit, int flags);
TRANSCRIPT_API transcript_error_t transcript_to_unicode_skip(transcript_t *handle,
                                                             const char **inbuf,
                                                             const char *inbuflimit);
TRANSCRIPT_API transcript_error_t transcript_from_unicode_skip(transcript_t *handle,
                                                               const char **inbuf,
                                                               const char *inbuflimit);
TRANSCRIPT_API transcript_error_t transcript_from_unicode_flush(transcript_t *handle, char **outbuf,
...
TRANSCRIPT_API void transcript_normalize_name(const char *name, char *normalized_name,
...
/** Minimum required size for an output buffer for ::transcript_to_unicode, if M:N conversion are
 * allowed. */
#define TRANSCRIPT_MIN_UNICODE_BUFFER_SIZE (4 * 20)
/** Minimum required size for an output buffer for ::transcript_from_unicode, if M:N conversion are
 * allowed. */
...
TRANSCRIPT_API size_t transcript_iconv(transcript_iconv_t cd, char **inbuf, size_t *inbytesleft,
