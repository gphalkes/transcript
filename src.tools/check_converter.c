...
static int iconv_convert(iconv_t handle, uint32_t codepoint, char *result, int *fallback) {
  ...
  if ((call_result = iconv(handle, &codepoint_ptr, &codepoint_len, &result, &result_len)) ==
      (size_t)-1) {
...
static int transcript_convert(transcript_t *handle, uint32_t codepoint, char *result,
                              int *fallback) {
    ...
    case TRANSCRIPT_FALLBACK:
      ...
      if (transcript_from_unicode(handle, &codepoint_ptr, codepoint_ptr + 4, &result, result_limit,
                                  TRANSCRIPT_FILE_START | TRANSCRIPT_ALLOW_PRIVATE_USE |
                                      TRANSCRIPT_END_OF_TEXT | TRANSCRIPT_ALLOW_FALLBACK) !=
          TRANSCRIPT_SUCCESS) {
        return -1;
        transcript_from_unicode_reset(handle);
      }
...
static uint32_t transcript_revert(transcript_t *handle, const char *seq, int length) {
  ...
  switch (transcript_to_unicode(
      handle, &seq, seq + length, &codepoint_ptr, codepoint_ptr + 20,
      TRANSCRIPT_FILE_START | TRANSCRIPT_ALLOW_PRIVATE_USE | TRANSCRIPT_END_OF_TEXT)) {
    case TRANSCRIPT_SUCCESS:
      break;
    default:
...
int main(int argc, char *argv[]) {
  ...
  for (i = option_start; i < 0x110000; i++) {
    ...
    transcript_result_length =
        transcript_convert(transcript_handle, i, transcript_result, &transcript_fallback);
    if (iconv_result_length != transcript_result_length ||
        (iconv_result_length >= 0 &&
         memcmp(iconv_result, transcript_result, iconv_result_length) != 0) ||
    ...
    if (transcript_result_length > 0 && !transcript_fallback) {
      if ((reverted = transcript_revert(transcript_handle, transcript_result,
                                        transcript_result_length)) != i) {
        result |= 4;
