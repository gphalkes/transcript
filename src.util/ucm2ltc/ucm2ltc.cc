...
void fatal(const char *fmt, ...) {
  ...
  va_end(args);
#ifdef DEBUG
  if (option_abort) abort();
#endif
  exit(EXIT_FAILURE);
}

static bool compare_string(const char *a, const char *b) { return strcmp(a, b) < 0; }

Ucm::tag_t string_to_tag(const char *str) {
  if (strcmp(str, "<code_set_name>") == 0) return UcmBase::CODE_SET_NAME;
  if (strcmp(str, "<uconv_class>") == 0) return UcmBase::UCONV_CLASS;
  if (strcmp(str, "<subchar>") == 0) return UcmBase::SUBCHAR;
  if (strcmp(str, "<subchar1>") == 0) return UcmBase::SUBCHAR1;
  if (strcmp(str, "<mb_cur_max>") == 0) return UcmBase::MB_MAX;
  if (strcmp(str, "<mb_cur_min>") == 0) return UcmBase::MB_MIN;
  if (strcmp(str, "<icu:charsetFamily>") == 0) return UcmBase::CHARSET_FAMILY;
  if (strcmp(str, "<transcript:internal>") == 0) return UcmBase::INTERNAL;
  return Ucm::IGNORED;
}
...
void parse_byte_sequence(char *charseq, vector<uint8_t> &store) {
  ...
  while (*charseq != 0) {
    ...
    if (value > 255 || value < 0) fatal("%s:%d: byte value out of range\n", file_name, line_number);
    store.push_back(value);
    if (*charseq == '+') charseq++;
  }
  if (store.size() > 31) fatal("%s:%d: character sequence too long\n", file_name, line_number);
}
...
void *safe_malloc(size_t size) {
  void *retval;
  if ((retval = malloc(size)) == NULL) OOM();
  return retval;
}
...
void print_state_machine(const vector<State *> &states) {
  for (size_t i = 0; i < states.size(); i++) {
    ...
    for (size_t j = 0; j < states[i]->entries.size(); j++) {
      ...
      if (states[i]->entries[j].next_state != 0) printf(":%x", states[i]->entries[j].next_state);
  ...
  */
  ...
  for (i = 0; i < length; i++) {
    for (j = 0; j < states[state]->entries.size(); j++) {
      if (!(charseq[i] >= states[state]->entries[j].low &&
            charseq[i] <= states[state]->entries[j].high))
        continue;

      value +=
          states[state]->entries[j].base +
          (uint32_t)(charseq[i] - states[state]->entries[j].low) * states[state]->entries[j].mul;
  ...
  if (ucm->variants.size() == 0) {
    if ((item = variant_map.find(ucm->variant.normalized_id)) != variant_map.end())
      fatal("%s:%s: Multiple variants with the same normalized ID (%s, %s, %s) specified\n",
            ucm->name, item->second->base->name, ucm->variant.normalized_id, ucm->variant.id,
            item->first);
    variant_map[ucm->variant.normalized_id] = &ucm->variant;
    return;
  }

  for (deque<Variant *>::iterator iter = ucm->variants.begin(); iter != ucm->variants.end();
       iter++) {
    if ((item = variant_map.find((*iter)->normalized_id)) != variant_map.end())
      fatal("%s:%s: Multiple variants with the same normalized ID (%s, %s, %s) specified\n",
            (*iter)->base->name, item->second->base->name, (*iter)->normalized_id, (*iter)->id,
            item->first);
...
int main(int argc, char *argv[]) {
  ...
  if (option_output_name != NULL) {
    ...
    size_t len;
    base_name = file_name;
    while (strpbrk(base_name, DIRSEPS) != NULL) base_name = strpbrk(base_name, DIRSEPS) + 1;
  ...
  fprintf(
      output,
      "TRANSCRIPT_EXPORT const char * const *transcript_namelist_%s(void) { return namelist; }\n",
