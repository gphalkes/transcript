...
static full_state_t *allocate_new_state(full_state_t **head, full_state_t **tail,
                                        const vector<State *> &states, int idx) {
  bool calculate_cost = true;
  full_state_t *current;
  size_t i;
  int j;

  if ((current = (full_state_t *)calloc(1, sizeof(full_state_t))) == NULL) OOM();

  /* Set pointers correctly for systems which use non-zero NULL ptr. */
  for (i = 0; i < 256; i++) current->entries[i].next_state = NULL;
  current->next = NULL;
  current->count = 1;
    ...
    for (j = states[idx]->entries[i].low; j <= states[idx]->entries[i].high; j++) {
      switch (states[idx]->entries[i].action) {
        case ACTION_VALID:
          ...
          current->entries[j].next_state =
              allocate_new_state(head, tail, states, states[idx]->entries[i].next_state);
        ...
        case ACTION_UNASSIGNED:
          ...
          current->entries[j].next_state =
              (full_state_t *)states[states[idx]->entries[i].next_state];
          break;
        case ACTION_SHIFT:
        case ACTION_ILLEGAL:
...
static void mark_entry(full_state_t *start, unsigned char *bytes, int length,
  ...
  switch (start->entries[*bytes].action & ~ACTION_FLAG_PAIR) {
    ...
    case ACTION_FINAL_LEN4_NOFLAGS:
      if ((mark_action & ~ACTION_FLAG_PAIR) != (start->entries[*bytes].action & ~ACTION_FLAG_PAIR))
        start->entries[*bytes].action = (action_t)(
            ACTION_FINAL | ((mark_action | start->entries[*bytes].action) & ACTION_FLAG_PAIR));
      else
        start->entries[*bytes].action =
            (action_t)(start->entries[*bytes].action |
...
static int count_states(full_state_t *ptr) {
  int states = 0;
  for (; ptr != NULL; ptr = ptr->next) states++;
  return states;
}
...
static bool can_merge(full_state_t *a, full_state_t *b) {
  ...
  if ((a->flags & State::INITIAL) || (b->flags & State::INITIAL)) return false;

  for (i = 0; i < 256; i++) {
    switch (a->entries[i].action & ~ACTION_FLAG_PAIR) {
      case ACTION_SHIFT:
      case ACTION_ILLEGAL:
        if (a->entries[i].action != b->entries[i].action) return false;
        if (a->entries[i].next_state != b->entries[i].next_state) return false;
        break;
      case ACTION_FINAL_NOFLAGS:
      case ACTION_FINAL:
      case ACTION_FINAL_LEN1_NOFLAGS:
...
static int calculate_state_cost(full_state_t *state, Ucm::StateMachineInfo *info) {
  ...
  for (i = 0; i < 256; i++) {
    if (last_action != state->entries[i].action) cost += 8;
    switch (state->entries[i].action) {
      case ACTION_ILLEGAL:
      case ACTION_SHIFT:
      case ACTION_UNASSIGNED:
...
static int calculate_merge_cost(full_state_t *a, full_state_t *b, Ucm::StateMachineInfo *info) {
  ...
  for (i = 0; i < 256; i++) {
    switch (a->entries[i].action & ~ACTION_FLAG_PAIR) {
      case ACTION_ILLEGAL:
      case ACTION_SHIFT:
        if (a->entries[i].action != b->entries[i].action) PANIC();
        break;
      ...
      case ACTION_FINAL:
        if (b->entries[i].action == ACTION_UNASSIGNED && !info->unassigned_needs_flags())
          break;
        else if ((a->entries[i].action & ~ACTION_FLAG_PAIR) !=
                 (b->entries[i].action & ~ACTION_FLAG_PAIR)) {
          tmp_state.entries[i].action = (action_t)(
              ACTION_FINAL | ((a->entries[i].action | b->entries[i].action) & ACTION_FLAG_PAIR));
          if ((a->entries[i].action & ~ACTION_FLAG_PAIR) != ACTION_FINAL) extra_cost += a->count;
          if ((b->entries[i].action & ~ACTION_FLAG_PAIR) != ACTION_FINAL) extra_cost += b->count;
        } else
          tmp_state.entries[i].action =
              (action_t)(tmp_state.entries[i].action |
...
static void merge_states(full_state_t **tail, full_state_t *left, full_state_t *right,
    ...
    switch (left->entries[i].action & ~ACTION_FLAG_PAIR) {
      ...
      case ACTION_FINAL:
        if (right->entries[i].action == ACTION_UNASSIGNED && !info->unassigned_needs_flags())
          break;
        else if ((left->entries[i].action & ~ACTION_FLAG_PAIR) !=
                 (right->entries[i].action & ~ACTION_FLAG_PAIR))
          left->entries[i].action =
              (action_t)(ACTION_FINAL |
                         ((left->entries[i].action | right->entries[i].action) & ACTION_FLAG_PAIR));
        else
          left->entries[i].action =
              (action_t)(left->entries[i].action |
                         ((left->entries[i].action | right->entries[i].action) & ACTION_FLAG_PAIR));
        break;
      case ACTION_UNASSIGNED:
        if (info->unassigned_needs_flags()) {
          switch (right->entries[i].action & ~ACTION_FLAG_PAIR) {
            ...
            case ACTION_FINAL_NOFLAGS:
              left->entries[i].action =
                  (action_t)(ACTION_FINAL | ((left->entries[i].action | right->entries[i].action) &
                                             ACTION_FLAG_PAIR));
              break;
          }
        } else {
  ...
  if (right->count == 1) {
    ...
    for (ptr = *tail; ptr != NULL; ptr = ptr->prev) {
      for (i = 0; i < 256; i++)
        if (ptr->entries[i].next_state == right) ptr->entries[i].next_state = left;
    }
  }

  left->count += right->count;

  if (left->cost > 0) left->cost = calculate_state_cost(left, info);
  ...
  do {
    ...
    for (; head != NULL; head = head->next) {
      for (ptr = head->next; ptr != NULL; ptr = ptr->next) {
        if (memcmp(head->entries, ptr->entries, sizeof(full_entry_t) * 256) != 0) continue;
...
static void minimize_states(full_state_t *head, full_state_t **tail, Ucm::StateMachineInfo *info) {
  ...
  for (ptr = head; ptr != NULL; ptr = ptr->next)
    if (ptr->cost) ptr->cost = calculate_state_cost(ptr, info);

  // Fill cost list
  for (ptr = head; ptr != NULL; ptr = ptr->next) {
    for (subptr = ptr->next; subptr != NULL; subptr = subptr->next) {
      if (!can_merge(ptr, subptr)) continue;
  ...
  while (1) {
    if (option_verbose) fprintf(stderr, "\rStates remaining: %d   ", nr_states);
    ...
    for (list<merge_cost_t>::iterator iter = costs.begin(); iter != costs.end();) {
      ...
      if (iter->cost < best.cost) best = *iter;

      iter++;
    }

    if (nr_states <= 256 && best.cost > 0) break;
...
void minimize_state_machine(Ucm::StateMachineInfo *info, int flags) {
  ...
  if (option_verbose) fprintf(stderr, "Minimizing state machine\n");
  ...
  */
  ...
  for (ptr = head; ptr != NULL; ptr = ptr->next) {
    if (ptr->flags & State::INITIAL) continue;
  ...
  for (i = 0; i < nr_serialized_states; i++) {
    ...
    if (serialized_states[i]->flags & State::INITIAL) new_states.back()->flags |= State::INITIAL;

    last = 0;
    for (j = 1; j < 256; j++) {
      if (serialized_states[i]->entries[j].action == serialized_states[i]->entries[j - 1].action &&
          serialized_states[i]->entries[j].next_state ==
              serialized_states[i]->entries[j - 1].next_state)
        continue;

      new_states.back()->new_entry(
          Entry(last, j - 1, (int)(intptr_t)serialized_states[i]->entries[last].next_state,
